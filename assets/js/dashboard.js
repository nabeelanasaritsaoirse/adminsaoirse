  ...
  try {
    ...
    withdrawals.forEach((w) => {
      ...
      tr.innerHTML = `
        ...
        <td>
          <button class="btn btn-primary btn-sm" onclick="openWithdrawModal('${
            w._id
          }')">View</button>
        </td>
      `;
...
async function openWithdrawModal(id) {
  ...
  try {
    ...
    document.getElementById("modalApproveBtn").onclick = () =>
      approveWithdrawalGlobal(id);
    document.getElementById("modalRejectBtn").onclick = () =>
      rejectWithdrawalGlobal(id);
...
document
  .getElementById("withdrawTabBtn")
  ?.addEventListener("click", loadAllWithdrawals);
