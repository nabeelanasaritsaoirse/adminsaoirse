...
function setupEventListeners() {
  ...
  if (createFirstListBtn)
    createFirstListBtn.addEventListener("click", openCreateListModal);
  ...
  if (searchInput)
    searchInput.addEventListener("input", debounce(filterLists, 300));
    ...
    .map((list) => {
      ...
      return `
        <div class="list-card" data-list-id="${escapeHtml(list.listId)}">
          <div class="list-card-header" onclick="toggleListCard('${escapeHtml(
            list.listId
          )}')">
            ...
            <div>
              ...
              <i class="bi bi-chevron-down list-card-toggle" id="toggle-${escapeHtml(
                list.listId
              )}"></i>
            </div>
          </div>
          <div class="list-card-body" id="body-${escapeHtml(list.listId)}">
            ...
            <div class="list-info">
              <div class="list-info-row">
                ...
                <div class="list-info-item">
                  ...
                  <span class="list-info-value">${formatDate(
                    list.createdAt
                  )}</span>
                </div>
              </div>
              ${
                list.description
                  ? `<p class="mb-0 mt-2 text-muted">${escapeHtml(
                      list.description
                    )}</p>`
                  : ""
              }
            ...
            <div class="list-actions">
              <button class="btn btn-sm btn-primary btn-icon" onclick="openAddProductModal('${escapeHtml(
                list.listId
              )}')">
                <i class="bi bi-plus-circle"></i> Add Product
              </button>
              <button class="btn btn-sm btn-info btn-icon" onclick="syncAllProducts('${escapeHtml(
                list.listId
              )}')">
                <i class="bi bi-arrow-repeat"></i> Sync All
              </button>
              <button class="btn btn-sm btn-warning btn-icon" onclick="openEditListModal('${escapeHtml(
                list.listId
              )}')">
                <i class="bi bi-pencil"></i> Edit
              </button>
              <button class="btn btn-sm btn-danger btn-icon" onclick="openDeleteListModal('${escapeHtml(
                list.listId
              )}', 'list')">
            ...
            <div class="products-section">
              <div class="products-header">
                ...
                <span class="products-count">${productCount} ${
        productCount === 1 ? "product" : "products"
              ...
              <div id="products-${escapeHtml(list.listId)}">
                ${
                  productCount > 0
                    ? renderProducts(products, list.listId)
...
function renderProducts(products, listId) {
  ...
  return products
    .map((product, index) => {
      ...
      return `
        <div class="product-item" data-product-id="${escapeHtml(
          product.productId
        )}">
          <div class="product-order">${product.order || index + 1}</div>
          <img src="${escapeHtml(imageUrl)}" alt="${escapeHtml(
        product.productName
      )}" class="product-image" onerror="this.src='https://via.placeholder.com/60'" />
          <div class="product-details">
            <div class="product-name">${escapeHtml(product.productName)}</div>
            <div class="product-meta">
              <span><i class="bi bi-tag"></i> ${escapeHtml(
                product.productId
              )}</span>
              ${
                product.lastSynced
                  ? `<span><i class="bi bi-clock"></i> Synced ${formatDate(
                      product.lastSynced
                    )}</span>`
                  : ""
              }
          ...
          <div class="product-price">
            ${
              showDiscount
                ? `<span class="price-regular">${regularPrice}</span>`
                : ""
            }
            <span class="price-final">${finalPrice}</span>
          </div>
          <div class="product-actions">
            <button class="btn btn-sm btn-outline-primary btn-order" onclick="moveProductUp('${escapeHtml(
              listId
            )}', '${escapeHtml(product.productId)}', ${product.order})" ${
        product.order === 1 ? "disabled" : ""
      }>
              <i class="bi bi-arrow-up"></i>
            </button>
            <button class="btn btn-sm btn-outline-primary btn-order" onclick="moveProductDown('${escapeHtml(
              listId
            )}', '${escapeHtml(product.productId)}', ${product.order})" ${
        product.order === products.length ? "disabled" : ""
      }>
              <i class="bi bi-arrow-down"></i>
            </button>
            <button class="btn btn-sm btn-outline-info btn-order" onclick="syncProduct('${escapeHtml(
              listId
            )}', '${escapeHtml(product.productId)}')">
              <i class="bi bi-arrow-repeat"></i>
            </button>
            <button class="btn btn-sm btn-outline-danger btn-order" onclick="openDeleteProductModal('${escapeHtml(
              listId
            )}', '${escapeHtml(product.productId)}')">
...
function openCreateListModal() {
  ...
  document.getElementById("listDesign").value = "1";
...
function openEditListModal(listId) {
  ...
  document.getElementById("listIsActive").value = list.isActive
    ? "true"
    : "false";
  document.getElementById("listDesign").value = list.design || 1;
...
async function saveList() {
  try {
    ...
    const listDescription = document
      .getElementById("listDescription")
      .value.trim();
    const displayOrder =
      parseInt(document.getElementById("listDisplayOrder").value, 10) || 0;
    const isActive = document.getElementById("listIsActive").value === "true";

    // 🔹 NEW: design field (safe default = 1)
    const design =
      parseInt(document.getElementById("listDesign")?.value, 10) || 1;
    ...
    // 🔹 NEW: design validation (frontend guardrail)
    if (!Number.isInteger(design) || design < 1 || design > 5) {
      showNotification("Design must be a number between 1 and 5", "error");
      return;
    }
    ...
    if (listId) {
      ...
      response = await API.post(
        API_CONFIG.endpoints.featuredLists.create,
        data
      );
    }
    ...
    if (response.success) {
      showNotification(
        listId ? "List updated successfully" : "List created successfully",
        "success"
      );
...
function openDeleteListModal(listId, type) {
  ...
  const message =
    "Are you sure you want to delete this list? All products in this list will be removed.";
...
function openAddProductModal(listId) {
  ...
  document.getElementById("productSearchResults").innerHTML =
    '<div class="text-center p-3 text-muted">Start typing to search products...</div>';
  ...
  try {
    ...
    if (response.success && response.data && response.data.length > 0) {
      ...
      if (activeProducts.length === 0) {
        resultsContainer.innerHTML =
          '<div class="text-center p-3 text-muted">No active products found</div>';
        return;
      }
      ...
      resultsContainer.innerHTML = activeProducts
        .map((product) => {
          ...
          const regularPrice =
            product.pricing?.regularPrice || product.price || 0;
          const finalPrice =
            product.pricing?.finalPrice || product.finalPrice || regularPrice;
          const stockQuantity =
            product.availability?.stockQuantity || product.stockQuantity || 0;

          return `
            <div class="product-search-item" onclick="selectProduct('${escapeHtml(
              product.productId
            )}', '${escapeHtml(productName)}', '${escapeHtml(
            imageUrl
          )}', ${finalPrice})">
              <img src="${escapeHtml(imageUrl)}" alt="${escapeHtml(
            productName
          )}" class="product-search-image" onerror="this.src='https://via.placeholder.com/50'" />
              <div class="product-search-details">
                <div class="product-search-name">${escapeHtml(
                  productName
                )}</div>
      ...
      resultsContainer.innerHTML =
        '<div class="text-center p-3 text-muted">No products found</div>';
    }
  } catch (error) {
    console.error("Error searching products:", error);
    resultsContainer.innerHTML =
      '<div class="text-center p-3 text-danger">Error searching products</div>';
  }
}
...
function selectProduct(productId, productName, imageUrl, price) {
  ...
  previewInfo.innerHTML = `
    <div class="d-flex align-items-center gap-3">
      <img src="${escapeHtml(imageUrl)}" alt="${escapeHtml(
    productName
  )}" style="width: 60px; height: 60px; object-fit: cover; border-radius: 0.375rem;" onerror="this.src='https://via.placeholder.com/60'" />
      <div>
...
async function reorderProducts(listId, productsArray) {
  try {
    ...
    if (response.success) {
      ...
      showNotification(
        response.message || "Failed to reorder products",
        "error"
      );
    }
  } catch (error) {
...
function updateStats() {
  ...
  const totalProducts = allLists.reduce(
    (sum, l) => sum + (l.products ? l.products.length : 0),
    0
  );
  const avgProducts =
    totalLists > 0 ? (totalProducts / totalLists).toFixed(1) : 0;
